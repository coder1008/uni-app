...
class cls_mysql {
	...
	private $sql;                       //sql执行语句
	private $row;                       //返回的条目数
	private $mysql_charset;             //数据库编码
	private $bulletin = true;           //是否开启错误记录
	private $show_error = false;         //测试阶段，显示所有错误,具有安全隐患,默认关闭
	private $is_error = false;          //调试模式建议设置false

    /*初始化对象*/
	public function __construct($db_host, $port, $db_user, $db_pwd, $db_database, $mysql_charset) {
		$this->db_host = $db_host;
		$this->port = $port;
        $this->db_user = $db_user;
		$this->db_pwd = $db_pwd;
		$this->db_database = $db_database;
        $this->bm = $mysql_charset;
        $this->connect();
	}
	...
	public function connect() {
		if ($this->conn == "pconn") {
			//永久链接
			$this->conn = mysql_pconnect($this->db_host . ":" . $this->port, $this->db_user, $this->db_pwd);
		} else {
			//即使链接
			$this->conn = mysql_connect($this->db_host . ":" . $this->port, $this->db_user, $this->db_pwd);
		}
		if (!mysql_select_db($this->db_database, $this->conn)) {
			if ($this->show_error) {
				$this->show_error("该数据库不可用：", $this->db_database);
			}
		}
	...
	*/
	...
	public function get_one($sql) {
		...
		if($result) {
			$data = array();
			while( $row = $this->fetch_assoc_($result) ) {
				$data[] = $row;
			}
		}
	...
	public function fetch_array_($result) {
	...
	public function fetch_assoc_($result) {
